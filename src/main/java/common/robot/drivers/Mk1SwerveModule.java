...
import static common.robot.Constants.CAN_TIMEOUT_MS;

import common.drivers.SwerveModule;
 ...
 *
 * <p>This implementation matches what hardware was used on both the robot we made during the 2017
 * off-season and what was used on the 2018 competition robot. It assumes that two Talon SRXs are
 * used to control each module over CAN and that the drive Talon SRX is connected to a CIMcoder and
 * the angle Talon SRX is connected to an analog encoder.
 *
 * <p>The drive distance units default to inches. This can be changed using {@link
 * Mk1SwerveModule#setDriveTicksPerUnit(double)}
 */
public final class Mk1SwerveModule extends SwerveModule {
  /**
   * How many angle encoder ticks occur for one radian travel.
   *
   * <p>The angle encoder travels 1 to 1 with the module so one rotation of the angle encoder is one
   * rotation of the module. A Talon SRX gives us 1024 ticks per rotation for an analog encoder so
   * we can divide that by 2&pi; to get the ticks per radian.
   */
   ...
   *
   * <p>This value was taken from our 2018 robot.
   */
   ...
   */
  public Mk1SwerveModule(
      Vector2 modulePosition, double offsetAngle, TalonSRX angleMotor, TalonSRX driveMotor) {
    ...
    this.driveMotor.configSelectedFeedbackSensor(
        FeedbackDevice.CTRE_MagEncoder_Relative, 0, CAN_TIMEOUT_MS);
   ...
   *
   * <p>The amount of ticks per inch can be calculated by driving the robot some distance forwards
   * (10 feet is usually good) and then dividing the average module ticks by that distance.
   *
   * <p>The default value uses inches and should only be used for testing.
   *
