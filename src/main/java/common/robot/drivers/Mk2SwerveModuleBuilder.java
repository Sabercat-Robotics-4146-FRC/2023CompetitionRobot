...
//      */
//     private static final PidConstants DEFAULT_ONBOARD_NEO_ANGLE_CONSTANTS = new PidConstants(0.5,
// 0.0, 0.0001);

//     /**
//      * Default constants for angle pid running on-board with CIMs.
//      */
//     private static final PidConstants DEFAULT_ONBOARD_CIM_ANGLE_CONSTANTS = new PidConstants(0.5,
// 0.0, 0.0001);

//     /**
//      * Default constants for angle pid running on-board with Mini CIMs
//      */
//     private static final PidConstants DEFAULT_ONBOARD_MINI_CIM_ANGLE_CONSTANTS = new
// PidConstants(0.5, 0.0, 0.0001);

//     /**
//      * Default constants for angle pid running on a Spark MAX using NEOs.
//      */
//     private static final PidConstants DEFAULT_CAN_SPARK_MAX_ANGLE_CONSTANTS = new
// PidConstants(1.5, 0.0, 0.5);

//     private static final PidConstants DEFAULT_FALCON_ANGLE_CONSTANTS = new PidConstants(0.1, 0.0,
// 0.5);
...
//     /**
//      * Configures the swerve module to use an analog encoder through one of the RoboRIO's analog
// input ports.
//      *
//      * @param encoder The analog input handle to use for the encoder.
//      * @param offset  The offset of the encoder in radians. This value is added to the analog
// encoder reading to obtain
//      *                the true module angle.
//      * @return The builder.
//      */
//     public Mk2SwerveModuleBuilder angleEncoder(AnalogInput encoder, double offset) {
//         angleSupplier = () -> {
//             double angle = (1.0 - encoder.getVoltage() / RobotController.getVoltage5V()) * 2.0 *
// Math.PI;
//             angle += offset;
//             angle %= 2.0 * Math.PI;
//             if (angle < 0.0) {
...
//      * <p>
//      * The default PID constants and angle reduction are used. These values have been determined
// to work with all Mk2
//      * modules controlled by this motor.
//      * <p>
//      * To override this values see {@link #angleMotor(CANSparkMax, PidConstants, double)}
//      *
//      * @param motor The CAN Spark MAX to use as the angle motor. The NEO's encoder is set to
// output the module's angle
//      *              in radians.
//      * @return The builder.
//      */
...
//             return angleMotor(motor, DEFAULT_CAN_SPARK_MAX_ANGLE_CONSTANTS,
// DEFAULT_ANGLE_REDUCTION);
//         }
...
//      * Configures the swerve module to use a CAN Spark MAX driving a NEO as it's angle motor.
//      * <p>
//      * This method is usually used when custom PID tuning is required. If using the standard
// angle reduction
//      * and a NEO, {@link #angleMotor(CANSparkMax)} uses already tuned constants so no tuning is
// required.
//      *
//      * @param motor     The CAN Spark MAX to use as the angle motor. The NEO's encoder is set to
// output the module's
//      *                  angle in radians.
//      * @param constants The PID constants to use to control the module's angle (units are in
// radians).
...
//      * @return The builder.
//      */
//     public Mk2SwerveModuleBuilder angleMotor(CANSparkMax motor, PidConstants constants, double
// reduction) {
...
//     public Mk2SwerveModuleBuilder angleMotor(TalonFX motor, PidConstants constants, double
// reduction) {
...
//         targetAngleConsumer = targetAngle -> {
//             double currentAngle = sensorCoefficient *
...
//         };
//         initializeAngleCallback = angle ->
// motor.getSensorCollection().setIntegratedSensorPosition(angle / sensorCoefficient, 50);
...
//      * <p>
//      * The default PID constants are used. These values have been determined to work with all Mk2
// modules
...
//         updateCallbacks.add((module, dt) ->
// motor.set(controller.calculate(module.getCurrentAngle(), dt)));
...
//      * <p>
//      * The default reduction and wheel diameter are used. These will work with the recommended
// wheel and stock drive
//      * reduction. By default distance and velocity will be reported in inches and inches per
// second.
//      *
//      * @param motor The CAN Spark MAX to use as the drive motor. The NEO's encoder is set to
// output the module's driven
...
//      * Configures the swerve module to use a CAN Spark MAX driving a NEO as it's drive motor.
//      *
//      * @param motor         The CAN Spark MAX to use as the drive motor. The NEO's encoder is set
// to output the module's driven
//      *                      distance and current velocity. Units are the same as the units used
// for the wheel diameter.
//      * @param reduction     The drive reduction of the module. It should be specified so the
...
//      * @return The builder.
//      */
//     public Mk2SwerveModuleBuilder driveMotor(CANSparkMax motor, double reduction, double
// wheelDiameter) {
//         CANEncoder encoder = motor.getEncoder();
//         encoder.setPositionConversionFactor(wheelDiameter * Math.PI / reduction);
//         encoder.setVelocityConversionFactor(wheelDiameter * Math.PI / reduction * (1.0 / 60.0));
// // RPM to units per second
...
//     public Mk2SwerveModuleBuilder driveMotor(TalonFX motor, double reduction, double
// wheelDiameter) {
//         TalonFXConfiguration config = new TalonFXConfiguration();
//         motor.configAllSettings(config);
//         motor.setNeutralMode(NeutralMode.Brake);
...
// motor.getSensorCollection().getIntegratedSensorPosition()) / (2048.0 * reduction);
...
//     /**
//      * Configures the swerve module to use a generic speed controller driving the specified
// motor.
//      *
...
//         // Spark MAXs are special and drive brushed motors in the opposite direction of every
// other motor controller
//         if (motorType != MotorType.NEO && (motor instanceof Spark || motor instanceof
// CANSparkMax)) {
//             motor.setInverted(true);
//         }
...
//             throw new IllegalStateException("No absolute encoder has been configured! See
// Mk2SwerveModuleBuilder.angleEncoder");
//         } else if (driveOutputConsumer == null) {
//             // Drive motor not configured
//             throw new IllegalStateException("No drive motor has been configured! See
// Mk2SwerveModuleBuilder.driveMotor");
//         } else if (targetAngleConsumer == null) {
//             // Angle motor not configured
//             throw new IllegalStateException("No angle motor has been configured! See
// Mk2SwerveModuleBuilder.angleMotor");
//         }
