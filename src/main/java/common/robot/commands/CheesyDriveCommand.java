...
import common.robot.subsystems.TankDrivetrain;
...
public final class CheesyDriveCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (!(drivetrain instanceof ShiftingTankDrivetrain)
        || ((ShiftingTankDrivetrain) drivetrain).inHighGear()) {
    ...
    if (negInertiaAccumulator > 1) negInertiaAccumulator -= 1;
    else if (negInertiaAccumulator < -1) negInertiaAccumulator += 1;
    else negInertiaAccumulator = 0;
      ...
      if (Math.abs(linearPower) < QUICK_STOP_DEADBAND) {
        ...
        quickStopAccumulator =
            (1 - alpha) * quickStopAccumulator
                + alpha * MathUtils.clamp(wheel, -1, 1) * QUICK_STOP_SCALAR;
      }
      overPower = 1;
      angularPower = wheel;
    } else {
