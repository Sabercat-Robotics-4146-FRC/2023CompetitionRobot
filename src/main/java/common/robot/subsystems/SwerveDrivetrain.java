...
import common.util.InterpolatingTreeMap;
...
public abstract class SwerveDrivetrain extends HolonomicDrivetrain {
  ...
  private InterpolatingTreeMap<InterpolatingDouble, Vector2> positionSamples =
  ...
  public synchronized void updateKinematics(double timestamp) {
    ...
    for (SwerveModule module : swerveModules) {
      ...
      Vector2 estimatedCenter =
          new RigidTransform2(module.getCurrentPosition(), Rotation2.fromRadians(robotRotation))
              .transformBy(
                  new RigidTransform2(module.getModulePosition().inverse(), Rotation2.ZERO))
              .translation;
...

    {
      ...
      kinematicVelocity =
          averageCenter
              .subtract(lastPosition.getValue())
              .scale(1 / (timestamp - lastPosition.getKey().value));
    }
    kinematicPosition = averageCenter;

    for (SwerveModule module : swerveModules) {
      module.resetKinematics(
          new RigidTransform2(kinematicPosition, Rotation2.fromRadians(robotRotation))
              .transformBy(new RigidTransform2(module.getModulePosition(), Rotation2.ZERO))
              .translation);
  ...
  public void outputToSmartDashboard() {
    ...
    for (SwerveModule module : getSwerveModules()) {
      SmartDashboard.putNumber(
          String.format("%s module angle", module.getName()),
          Math.toDegrees(module.getCurrentAngle()));
      SmartDashboard.putNumber(
          String.format("%s module drive distance", module.getName()), module.getCurrentDistance());
      SmartDashboard.putString(
          String.format("%s module position", module.getName()),
          module.getCurrentPosition().toString());
      SmartDashboard.putNumber(
          String.format("%s module velocity", module.getName()), module.getCurrentVelocity());
      SmartDashboard.putNumber(
          String.format("%s module drive current", module.getName()),
          module.getDriveCurrent() + Math.random() * MathUtils.EPSILON);
    }
  }
}
