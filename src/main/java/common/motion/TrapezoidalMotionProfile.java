...
public final class TrapezoidalMotionProfile extends MotionProfile {
  ...
  private State direct(State in) {
    return new State(
        in.time, in.position * direction, in.velocity * direction, in.acceleration * direction);
  }
  ...
  public State calculate(double time) {
    ...
    if (time < endAccelerationTime) {
      ...
      position =
          start.position
              + (start.velocity + endAccelerationTime * constraints.maxAcceleration / 2)
                  * endAccelerationTime
      ...
      position =
          end.position - (end.velocity + timeLeft * constraints.maxAcceleration / 2) * timeLeft;
    } else {
      acceleration = 0;
