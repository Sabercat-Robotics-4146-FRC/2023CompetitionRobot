...
public class HolonomicFeedforward {
  ...
  public HolonomicFeedforward(
      DrivetrainFeedforwardConstants forwardConstants,
  ...
  public Vector2 calculateFeedforward(Vector2 velocity, Vector2 acceleration) {
    // We don't use `DrivetrainFeedforwardConstants.calculateFeedforward` because we want to apply
    // kS (the static
    ...
    forwardFeedforward +=
        Math.copySign(
            feedforwardUnitVector.x * forwardConstants.getStaticConstant(), forwardFeedforward);
    strafeFeedforward +=
        Math.copySign(
            feedforwardUnitVector.y * strafeConstants.getStaticConstant(), strafeFeedforward);
