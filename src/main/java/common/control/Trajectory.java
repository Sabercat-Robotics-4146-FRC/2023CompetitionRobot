...
public class Trajectory {
  ...
  public Trajectory(
    ...
    double distance = 0.0;
    ConstrainedPathState lastState =
        new ConstrainedPathState(
    ...
    while (distance < path.getLength()) {
      ...
      ConstrainedPathState state =
          new ConstrainedPathState(
              startingState, profileLength, startingVelocity, maxEndingVelocity, 0.0);

      // If the max ending velocity is lower than the starting velocity we know that we have to
      // decelerate
      double maxDeltaVelocity = maxEndingVelocity - startingVelocity;

      // Calculate the optimal acceleration for this profile
      double optimalAcceleration =
          Math.pow(maxDeltaVelocity, 2.0) / (2.0 * profileLength)
      ...
      if (MathUtils.epsilonEquals(optimalAcceleration, 0.0)) {
        ...
        for (TrajectoryConstraint constraint : trajectoryConstraints) {
          maxStartingAcceleration =
              Math.min(
                  constraint.getMaxAcceleration(startingState, startingVelocity),
                  maxStartingAcceleration);
        ...
        // This acceleration may not be achievable. When we go over the trajectory in reverse we
        // will take care
        // of this.
      ...
      if (deltaVelocity < 0.0) {
        ...
        for (TrajectoryConstraint constraint : trajectoryConstraints) {
          deceleration =
              Math.min(
                  deceleration,
                  constraint.getMaxDeceleration(
                      constrainedState.pathState, constrainedState.endingVelocity));
        }
        ...
        double decelDist =
            0.5 * deceleration * Math.pow(decelTime, 2.0)
  ...
  class ConstrainedPathState {
    ...
    public State calculate(double time) {
      ...
      double distance =
          0.5 * acceleration * Math.pow(time, 2.0)
