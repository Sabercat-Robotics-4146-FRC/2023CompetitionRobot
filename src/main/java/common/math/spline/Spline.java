...
public class Spline {
  ...
  public Spline derivative() {
    // The derivative is used when calculating the tangent or curvature. Cache it so we don't have
    // to calculate it
    // multiple times.
    if (derivative == null) {
      SimpleMatrix coefficients = basisMatrix.mult(basisWeightMatrix);
      SimpleMatrix derivativeMatrix =
          new SimpleMatrix(coefficients.numRows() - 1, coefficients.numRows());
      for (int i = 0; i < derivativeMatrix.numRows(); i++) {
        derivativeMatrix.set(i, i + 1, i + 1);
      }

      derivative =
          new Spline(SimpleMatrix.identity(getDegree()), derivativeMatrix.mult(coefficients));
    }
